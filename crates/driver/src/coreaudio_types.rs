//! CoreAudio C type bindings for the plug-in (forked from MARS `mars-hal`).
//!
...
// ---------------------------------------------------------------------------
// Custom property selectors (Patchbay)
// ---------------------------------------------------------------------------

pub const K_PB_PROPERTY_DESIRED_STATE: UInt32 = fourcc(b"pbds");
// Avoid collision with CoreAudio's deprecated `kAudioHardwarePropertyProcessIsMaster` (`'mast'`).
pub const K_PB_PROPERTY_APPLIED_STATE: UInt32 = fourcc(b"pbas");
pub const K_PB_PROPERTY_RUNTIME_STATS: UInt32 = fourcc(b"pbrs");
pub const K_PB_PROPERTY_CONFIG_SUMMARY: UInt32 = fourcc(b"pbcf");
...
pub struct AudioServerPlugInIOCycleInfo {
    ...
    pub m_main_host_ticks_per_frame: Float64,
    pub m_device_host_ticks_per_frame: Float64,
}

/// `SMPTETime` (CoreAudioBaseTypes.h).
#[repr(C)]
...
/// `AudioTimeStamp` (CoreAudioBaseTypes.h).
#[repr(C)]
#[derive(Debug, Clone, Copy, Default)]
pub struct AudioTimeStamp {
    pub m_sample_time: Float64,
    pub m_host_time: u64,
    pub m_rate_scalar: Float64,
...
pub const PATCHBAY_DRIVER_BUNDLE_ID: &str = "app.fasttrackstudio.patchbay.driver";
...
mod tests {
    ...
    fn io_cycle_info_matches_apple_layout() {
        assert_eq!(mem::size_of::<AudioTimeStamp>(), 64);
        // AudioServerPlugIn.h: counter, nominal size, current, input,
        // output timestamps, then two Float64 tick rates.
        assert_eq!(
            mem::offset_of!(AudioServerPlugInIOCycleInfo, m_current_time),
            16
        );
        assert_eq!(
            mem::offset_of!(AudioServerPlugInIOCycleInfo, m_input_time),
            80
        );
        assert_eq!(
            mem::offset_of!(AudioServerPlugInIOCycleInfo, m_output_time),
            144
        );
        assert_eq!(mem::size_of::<AudioServerPlugInIOCycleInfo>(), 224);
    }

    #[test]
    fn fourcc_values() {
        assert_eq!(K_PB_PROPERTY_DESIRED_STATE, fourcc(b"pbds"));
        assert_eq!(K_PB_PROPERTY_APPLIED_STATE, fourcc(b"pbas"));
        assert_eq!(K_PB_PROPERTY_RUNTIME_STATS, fourcc(b"pbrs"));
        assert_eq!(K_PB_PROPERTY_CONFIG_SUMMARY, fourcc(b"pbcf"));
